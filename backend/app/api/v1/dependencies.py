...
def current_session(request: Request, services: Annotated[Container, Depends(container)]) -> Session:
    ...
    now = time.time()
    session = services.store.find_session(token_hash(token), now)
    if session is None:
        raise AppError("session_required", "Сессия истекла. Создайте новую.", 401)
    # The per-IP bucket collapses to one bucket behind the frontend proxy, so the
    # per-session bucket is what actually bounds a single client. Charged once per
    # request: mutation_session depends on this and must not charge again.
    services.store.consume_rate("session:" + session.id, services.settings.requests_per_minute, now)
    return session
