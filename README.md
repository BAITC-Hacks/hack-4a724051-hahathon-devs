...
**Для самостоятельной технической проверки начните с раздела «Установка и запуск».** Автономный сценарий воспроизводится из репозитория без ключей EKT/OpenAI, PostgreSQL и антивируса. Подключение подготовленной PostgreSQL-базы и OpenAI описано отдельно. Демонстрационная корзина не оформляет заказ на EKT.
...
3. Отправьте `990100015_`, затем `есть аналог?`. При нулевом остатке ожидаются доступные подходящие аналоги с объяснением либо уточнение, если данных недостаточно.
...
Проверка 23.09.2026: **215 backend-тестов прошли, 16 PostgreSQL-тестов пропущены; 5 тестов proxy, TypeScript и сборка Next.js прошли.** Подробности и ограничения: [отчёт](docs/verification-2026-09-23.md). Тесты не гарантируют отсутствия всех ошибок; внешние зависимости проверяются отдельно.
