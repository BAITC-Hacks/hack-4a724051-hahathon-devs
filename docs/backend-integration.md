...
Платная генерация модели в HTTP runtime выключена и не активируется наличием ключа. Пришедший доменный AnthropicProvider сохраняется и тестируется без реальных запросов, но не обходит блокировку runtime. Перед его включением нужны общий бюджет, проверка политики данных и согласованные ограничения. Файловая загрузка, scanner, парсеры и PostgreSQL-адаптер ещё требуют интеграции; UI должен показывать недоступность вместо ложного успеха.
