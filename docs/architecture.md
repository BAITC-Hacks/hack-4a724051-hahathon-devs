...
Статус: проект архитектуры для реализации за 5 часов. Описанные ниже модули, endpoint-ы и механизмы ещё не реализованы; созданные документы и конфигурационные шаблоны не являются работающим backend. Контракт внешнего API: [api.md](api.md). Порядок реализации: [mvp-plan.md](mvp-plan.md).

## 1. Решения и границы MVP

**Модульный монолит:** Python/FastAPI для API, React/TypeScript для интерфейса, PostgreSQL для состояния и поискового индекса. Один backend-код запускается в двух процессах: HTTP API и worker. PostgreSQL используется также для небольшой устойчивой очереди задач. Файлы обрабатываются worker в изолированном дочернем процессе с пределами ресурсов. Для локального демо — приватный volume; интерфейс хранилища позволяет позже подключить S3-совместимый сервис.
...
```mermaid
flowchart TD
    UI[React: каталог, чат, демо-корзина] --> API[FastAPI: сессия, CSRF, лимиты, HTTP-контракты]
  ...
  app/
    ...
    modules/
      ...
      chat/
        ...
        service.py                # submit_turn(), get_turn(), cancel_turn()
