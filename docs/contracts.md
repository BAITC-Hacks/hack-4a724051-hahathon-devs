...
Обновление: эти dataclass-интерфейсы относятся к доменному слою. В рабочем HTTP-чате теперь используется OpenAI через `GroundedAssistant`; ключ, квоты, загрузки и способы проверки описаны в [assistant-testing.md](assistant-testing.md). Ниже приведена историческая конфигурация Anthropic; автоматическое создание платного провайдера из legacy factory отключено, чтобы не обходить общий бюджет.

Этот документ описывает пришедшие доменные интерфейсы поиска, аналогов, корзины и ассистента. Текущее распределение ответственности и их подключение к HTTP API описаны в [backend-integration.md](backend-integration.md). Пользователь (участник 2) владеет всем frontend на Next.js/TypeScript, дальнейшей работой над ИИ и своей частью backend. Примеры ChatReply ниже — внутренний доменный формат; браузер использует [OpenAPI](openapi.json).

Доменную логику можно запускать и тестировать с реализациями в памяти (`backend/app/adapters/memory.py`). Они не являются устойчивым хранилищем для нескольких процессов API/worker.

## Порты, которые подключаются к основной БД и источникам
