...
import { attributeValue, canAdd, displayAttribute, displayCategory, displayWarnings, money, safeCertificate, safeImage, type StoreProduct } from "./types";
import { useDialog } from "./use-dialog";

export function categoryIcon(name: string): IconName {
  const lower = name.toLowerCase();
  if (lower.includes("кабел") || lower.includes("провод") || lower.includes("kabel_provod")) return "cable";
  if (lower.includes("свет") || lower.includes("ламп") || lower.includes("svetilniki_lampy")) return "lamp";
  if (lower.includes("розет") || lower.includes("выключат") || lower.includes("rozetki_vyklyuchateli")) return "socket";
  ...
  if (lower.includes("защит") || lower.includes("автомат") || lower.includes("nizkovoltnaya")) return "shield";
...
export function ProductCard({ product, onOpen, onAdd, onFavorite, onCompare, favorite, compared, mode = "grid", cartReady }: {
  ...
  return <article className={`store-product ${mode}`}>
    ...
    <div className="store-product-info"><div className="product-meta"><span>{product.brand || displayCategory(product.category_path?.[0] || "Каталог ЭКТ")}</span><span className="product-article">{product.article_original}</span></div><button className="product-title" type="button" onClick={onOpen}>{product.name}</button><div className="product-specs">{product.attributes.filter(item => item.value && item.status !== "unknown").slice(0, mode === "list" ? 4 : 2).map(item => <span key={item.key}><b>{displayAttribute(item.key)}</b> {attributeValue(item.value, item.unit)}</span>)}</div><div className="product-purchase"><div><strong>{money(product.price_amount, product.price_currency || "KZT")}</strong><span className={`stock-state ${product.stock_status}`}>● {stock}</span></div><button className="add-cart-button" type="button" disabled={!cartReady || !canAdd(product)} onClick={onAdd} title={!cartReady ? "Корзина пока недоступна" : !canAdd(product) ? "Цена или остаток не подтверждены" : "Добавить в подборку"}><Icon name="cart" size={18}/><span>В подборку</span></button></div></div>
  </article>;
}

export function ProductDetail({ product, onClose, onAdd, onAsk, cartReady, onFavorite, favorite }: { product: StoreProduct; onClose: () => void; onAdd: () => void; onAsk: () => void; cartReady: boolean; onFavorite: () => void; favorite: boolean }) {
  const dialogRef = useDialog(true, onClose);
  return <div className="overlay" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><section ref={dialogRef} className="detail-modal" role="dialog" aria-modal="true" aria-label={product.name}><button data-dialog-close className="modal-close" type="button" onClick={onClose} aria-label="Закрыть"><Icon name="close"/></button><div className="detail-visual"><ProductVisual product={product} large/></div><div className="detail-copy"><span className="micro-label">КАТАЛОГ / {product.category_path?.map(displayCategory).join(" / ") || "ТОВАР"}</span><h2>{product.name}</h2><p className="detail-code">Артикул: {product.article_original}{product.brand ? ` · ${product.brand}` : ""}</p><div className={`detail-stock ${product.stock_status}`}>{product.stock_status === "available" ? "● В наличии" : product.stock_status === "out_of_stock" ? "● Нет в наличии" : "● Остаток неизвестен"}{product.sellable_quantity ? ` · ${product.sellable_quantity} ${product.unit || "шт."}` : ""}</div><div className="detail-attrs">{product.attributes.map((attribute, index) => <div key={`${attribute.key}-${index}`}><span>{displayAttribute(attribute.key)}</span><strong>{attributeValue(attribute.value, attribute.unit)}{attribute.status === "conflict" ? " · данные расходятся" : ""}</strong></div>)}</div>{displayWarnings(product.warnings).length > 0 && <p className="detail-warning">{displayWarnings(product.warnings).join(" ")}</p>}{product.certificates && product.certificates.length > 0 && <div className="detail-certs"><span className="micro-label">ДОКУМЕНТЫ</span>{product.certificates.map((certificate, index) => { const url = safeCertificate(certificate.url); return url ? <a href={url} key={index} target="_blank" rel="noreferrer">{certificate.title}{certificate.number ? ` №${certificate.number}` : ""} <Icon name="external" size={14}/></a> : null; })}</div>}<div className="detail-bottom"><strong>{money(product.price_amount, product.price_currency || "KZT")}</strong><button type="button" onClick={onFavorite} className="detail-favorite" aria-label={favorite ? "Убрать из избранного" : "В избранное"}><Icon name="heart"/></button><button className="main-button" type="button" disabled={!cartReady || !canAdd(product)} onClick={onAdd}><Icon name="cart" size={17}/> В подборку</button></div><button className="detail-ask" type="button" onClick={onAsk}><Icon name="spark" size={16}/> Спросить помощника об этом товаре</button><p className="micro-note">Корзина изменится только после проверки предложения и вашего подтверждения.</p></div></section></div>;
}
